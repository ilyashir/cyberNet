...
int* getAvailablePlaces(IplImage *image){
	...
	int k;
	int l;
	int *arr = new int[16];//массив для отображения доступных мест: 1 -- свободно, 0 -- занято
	for(int i = 0; i<16; i++){
		arr[i] = 0;
	}
...
int getBestPlace(int *arr){
	...
	return index + 1;//нумерация мест на парковке начинается с единицы, если свободных мест нет, возвращаем ноль
}
...
int main(){
	...
	char *s = "9:keepalive";//сообщение, автоматически посылаемое ТРИКом в течение всего времени соединения
	int n = 0;
